...
if (userDetails1 == null) {
	...
	"Please login first to see all notes...");
...
<body>
	...
	<%
				String noteEvent = (String) session.getAttribute("EventOnNote");
				if (noteEvent != null) {
				%>

				<div class="alert alert-info" role="alert"><%=noteEvent%></div>
				<%
	...
	<div class="container-fluid p-0 mt-3">
		<h1>
			All notes by: <span class="text-info"> <%=userDetails1.getName().toUpperCase()%></span>
		</h1>
		<p>Feel free to edit and delete any of them, they are all your</p>
		<div class="masonry">
			<%
			NoteDAO theirNotes = new NoteDAO(DBConnection.getCon());
			List<UserNotes> notes = theirNotes.getNotes(userDetails1.getId());
			...
			<div class="item">
				...
				<p>
					...
					<a href="editNote.jsp?note_id=<%=i.getId()%>"
						class="btn btn-primary btn-lg px-3 py-1 fs-6" role="button"
						aria-pressed="true">Edit</a> 
						<a href="DeleteNoteServlet?note_id=<%=i.getId()%>&uid=<%=userDetails1.getId()%>"
						class="btn btn-secondary btn-lg px-3 py-1 fs-6" role="button"
		...
		<div class="text-center">
			<a href="addNotes.jsp"
				class="btn btn-outline-success text-center mb-3 mx-auto px-4 fs-4">Add Note</a>
		</div>
	</div>
